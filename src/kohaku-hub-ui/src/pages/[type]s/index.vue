...
const rules = {
  name: [
    { required: true, message: `Please enter ${repoType.value} name`, trigger: 'blur' },
    { 
      pattern: /^[a-zA-Z0-9_-]+$/, 
...
async function handleCreate() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      const repoId = data.repo_id || `${form.organization || currentUser.value}/${form.name}`
      router.push(`/${repoType.value}s/${repoId}`)
    } catch (err) {
      ElMessage.error(err.response?.data?.detail || `Failed to create ${repoType.value}`)
    } finally {
