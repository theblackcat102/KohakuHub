    ...
    <div v-else>
      ...
      <div class="card">
        ...
        <div v-else-if="isAudio" class="text-center py-8">
          <div class="i-carbon-music text-6xl text-gray-400 mb-4 inline-block" />
        ...
        <div v-else-if="isMarkdown">
          <div class="flex items-center justify-between mb-4 pb-3 border-b border-gray-200">
            ...
            <el-button v-if="markdownView === 'source'" size="small" @click="copyContent">
              <div class="i-carbon-copy inline-block mr-1" />
              Copy
            </el-button>
          </div>
...
const languageLabel = computed(() => {
  const langMap = {
    ...
    bash: 'Bash', sql: 'SQL', r: 'R', m: 'MATLAB', jl: 'Julia'
  }
...
const isImage = computed(() => {
  return ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'bmp', 'ico'].includes(fileExtension.value)
})
...
const isAudio = computed(() => {
  return ['mp3', 'wav', 'ogg', 'flac', 'm4a', 'aac'].includes(fileExtension.value)
})
...
const isTextFile = computed(() => {
  const textExtensions = [
    'txt', 'log', 'csv', 'tsv',
    'js', 'ts', 'jsx', 'tsx', 'vue', 'py', 'java', 'cpp', 'c', 'h', 'hpp',
    'cs', 'go', 'rs', 'rb', 'php', 'swift', 'kt', 'scala', 'm', 'r', 'jl',
  ...
  return textExtensions.includes(fileExtension.value) || fileName.value.toLowerCase().includes('dockerfile')
})

const canPreviewText = computed(() => {
  return isTextFile.value && fileSize.value <= maxPreviewSize && !isMarkdown.value
})
...
function getFileIcon(filename) {
  ...
  if (['js', 'ts', 'jsx', 'tsx'].includes(ext)) return 'i-carbon-code text-yellow-500'
  if (['py'].includes(ext)) return 'i-carbon-code text-blue-600'
  if (['json', 'xml', 'yaml', 'yml'].includes(ext)) return 'i-carbon-data-structured text-orange-500'
  if (['zip', 'tar', 'gz', 'rar', '7z'].includes(ext)) return 'i-carbon-zip-archive text-gray-500'
...
function navigateToFolder(folderPath) {
  ...
  router.push(`/${repoType.value}s/${namespace.value}/${name.value}/tree/${branch.value}/${folderPath}`)
}
